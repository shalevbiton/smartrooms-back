...
app.post('/api/bookings', async (req, res) => {
  try {
    ...
    if (error && (error.code === '42703' || error.message.includes('column') || error.message.includes('booking_type'))) {
      console.log("Fallback: booking_type missing in DB, retrying without it...");
      delete dbData.booking_type;
      const retry = await supabase.from('bookings').insert(dbData).select().single();
      data = retry.data;
      error = retry.error;
    }
